...
def is_taxi_order_ai(text: str) -> dict:
    ...
    for model_name in models_to_try:
        ...
        # Tarmoq uzilishlari (WinError 10054) bo'lsa qayta urinib ko'rish
        for retry in range(2):
            try:
                ...
                err_str = str(e)
                # Agar vaqtinchalik tarmoq uzilishi bo'lsa, biroz kutib qayta urinamiz
                if ("10054" in err_str or "ServerDisconnectedError" in err_str or "RemoteProtocolError" in err_str) and retry == 0:
                    time.sleep(0.5)
                    continue
                
                logging.warning(f"AI Call Xatolik (Model: {model_name}, Key #{_current_client_index + 1}): {err_str[:120]}. Zaxira key/modelga o'tilmoqda...")
                rotate_client()
                break
